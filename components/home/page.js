...
function timeAgo(dateString) {
  ...
  // Handle future dates - show as "Just now" for now, or you can customize this
  if (diffInMs < 0) {
    // For future dates, just show "Just now" to avoid confusion
    // You can uncomment the lines below if you want to show "in X time" format
    /*
...
const Home = () => {
  ...
  const fetchConfessions = useCallback(async (pageNum) => {
    ...
    try {
      ...
      if (data && data.data) {
        const filtered = data.data.filter((c) => c.status === 1 || c.status === true);
  ...
  return (
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center pt-2 pb-20">
      ...
      {openCommentId && (
        ...
        >
          <div
            className={`w-full max-w-md h-[75vh] bg-gradient-to-b from-zinc-900/95 to-black/95 backdrop-blur-xl border border-rose-300/20 rounded-t-3xl p-5 flex flex-col shadow-2xl transform transition-transform duration-300 ease-out ${
